...
Points farming on perp DEXes is capital-inefficient. You deposit capital, lock it in a strategy, earn points alongside yield, and wait to see how the points program develops. You cannot separate yield from points, and you cannot choose the exact exposure you want.

## The Solution

We wrap perp DEX strategies into a **vToken** — a vault share that earns yield and accrues credits for supported points programs. Hold vToken for the combined exposure, or use your vTokens to buy **ST** (principal + yield) or **EPT** (rewards side) on the orderbook.

Points farmers buy EPT for rewards-side exposure. Yield seekers buy ST at a discount for principal + yield exposure. Each user chooses the side they want, at a price set by the market.
...
<CardGroup cols={3}>
  ...
  <Card title="Just Hold vToken" icon="wallet">
    Earn yield and accrue credits for supported points programs in one token. The simplest way to use ArcX.
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Deposit USDC">
    Deposit USDC into a vault. You receive **vToken** shares at the current NAV. Your vToken earns yield and accrues credits for supported points programs.
  </Step>
...
At the same time, those market maker strategies generate points on the underlying venues. ArcX tracks user credits on-chain. Those credits generate CreditTokens, and market makers can share rewards received from the underlying protocol with CreditToken holders.
...
See [Credit Mathematics →](/deep-dives/credit-mathematics) for the details of how credits accrue and how CreditTokens are split across users. See [Risk Disclosure →](/legal/risk-disclosure) for points-program and reward-distribution risks.
