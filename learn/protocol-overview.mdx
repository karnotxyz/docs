...
On platforms like TreadFi, you **burn capital for points**. Deposit $100 into a protocol, watch it earn near-zero yield, and hope the airdrop makes up for it. Your capital is locked, illiquid, and doing nothing productive.
...
- **[ST (Strategy Token)](/learn/strategy-token):** your claim on the USDC value at maturity
- **[EPT (Expected Points Token)](/learn/expected-points-token):** your claim on exchange points earned during the epoch
...
The flash loop is ArcX's core innovation for points farmers. It works atomically:

1. Deposit $100 → receive 100 EPT + ST
2. Sell ST on the ArcX AMM at ~10% discount → receive ~$90
3. Re-deposit $90 → receive 90 EPT + ST
4. Sell ST again → receive ~$81
5. Repeat...

**Result:** ~$270 of EPT exposure from a $100 deposit. Your effective cost per EPT is dramatically lower than depositing and holding.
...
**TradFi analogies:** ST = **closed-end fund share**. EPT = **pre-market points futures**. PointsToken = **gift card** redeemable at TGE.
...
**Epochs:** ArcX runs in fixed epoch windows. ACTIVE → MATURITY → FINALIZED. No automatic rollover: redeem from one epoch, deposit into the next. See [How Epochs Work](/learn/epoch-lifecycle).

**Trust model:** ArcX operates all oracles, executes the strategy, and backs PointsTokens. This is a centralized system that trades trustlessness for speed to market. Every trust assumption is documented. See [Trust Model & Security](/deep-dives/trust-model-and-security).
