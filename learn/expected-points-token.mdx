...
After multiple iterations, you hold ~270 EPT from a \$100 starting deposit. Your net outlay is ~\$27 (\$100 minus the ~\$73 from the final ST sale), so your effective cost per EPT is ~\$0.10 instead of \$1.00 from a single deposit.

**Why this works:** Each deposit creates EPT (which you keep) and ST (which you sell at a discount). The ST discount is the "price" you pay for points. The flash loop amplifies this by recycling the ST sale proceeds. For the full mathematical derivation and convergence formula, see [EPT Pricing & Flash Loop Economics](/deep-dives/ept-pricing).
