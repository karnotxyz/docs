...
**The core idea:** Need to know when to deposit, when to trade, and when you get your money back? Every ArcX strategy runs in fixed epochs: ACTIVE → MATURITY → FINALIZED. What you can do depends on which state the epoch is in.
</Info>

**Prerequisites:** [What is ArcX?](/learn/protocol-overview), [The Three Tokens](/learn/token-economics)
...
**Why fixed windows?** Fixed epochs create clean boundaries: clear start/end times for credit accrual, predictable maturity dates for traders, and isolated risk per period. They also simplify the contract model. Each epoch is a self-contained unit with its own state.
...
For the full deposit validation pseudocode, see [Developer Integration Guide](/build/developer-integration-guide).
...
Each epoch has a potentially different risk profile. Market conditions change across epochs. The strategy's exposure, exchange counterparty risk, and points programs may shift. Requiring explicit re-deposit ensures:

- You make a conscious decision to re-enter
...
<AccordionGroup>
<Accordion title="Can I stay in the next epoch automatically?">
No. There is no automatic rollover. You must redeem from the current epoch and deposit into the next one. This is by design. Each epoch may have different risk conditions, and you should actively choose to re-enter.
</Accordion>
</AccordionGroup>
...
Pause is a conservative measure: it stops new money from entering while existing positions continue unchanged. This is appropriate for situations where the strategy may need attention but existing depositors aren't at immediate risk.
...
For detailed sequence diagrams of every operation (deposit, cross-chain deposit, refund, finalization, redemption, claiming), see [Integration & Deployment](/build/integration-and-deployment).
