...
stateDiagram-v2
    ...
    state ACTIVE {
        [*] --> Deposits\ open
        Deposits\ open --> Credits\ accruing
        Credits\ accruing --> ST\ trading\ on\ AMM
        ST\ trading\ on\ AMM --> Flash\ loop\ available
    }

    state MATURITY {
        [*] --> Strategy\ unwinding
        Strategy\ unwinding --> Settling\ positions
        Settling\ positions --> No\ new\ deposits
    }

    state FINALIZED {
        [*] --> ST\ redeemable\ for\ USDC
        ST\ redeemable\ for\ USDC --> EPT\ claimable\ for\ PointsTokens
        EPT\ claimable\ for\ PointsTokens --> No\ expiry
    }
```
