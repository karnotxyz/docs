...
ST is your claim on the strategy's USDC returns at maturity. Deposit USDC, receive ST shares. Hold them, sell them on the AMM, or redeem them when the epoch finalizes.
...
The ArcX AMM price typically tracks slightly below NAV (reflecting the ST discount from flash loop selling pressure), then converges toward NAV as maturity approaches via the time-decay curve. For the full price trajectory chart and analysis, see [EPT Pricing: ST Price Through the Epoch](/deep-dives/ept-pricing#st-price-through-the-epoch).
