...
**The core idea:** Deposit USDC, receive two tokens, use the flash loop for leveraged points or buy discounted ST for yield. Redeem after finalization. That's the full loop.
</Info>
...
| **Which exchanges** | Determines which EPTs you receive (EPT_Pacifica, EPT_Extended, etc.) and which points you earn. |
| **Current epoch** | Is the deposit window open? How much time remains? Depositing late means fewer credits on your EPT. |
...
<Steps>
  ...
  <Step title="Hold until maturity">
    The strategy earns ~1% over the epoch. ST redeems at ~\$1.01.
  </Step>
  <Step title="Redeem after finalization">
    Burn ST → receive \$1.01 per share. Your return: (\$1.01 - \$0.90) / \$0.90 = **12.2% in one epoch**.
  </Step>
</Steps>
...
- **Strategy performance:** NAV chart, current creditRate
...
| **Trade** | PointsTokens are standard ERC20s, tradeable on any Starknet DEX. |
