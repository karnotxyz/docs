...
**The core idea:** Already know Pendle? Good: the concepts map closely. Both split an asset into two tradeable tokens. But they split different things, the tokens behave differently at maturity, and the pricing dynamics diverge in important ways. ArcX builds on Pendle's model for perp DEX points markets. This page maps every Pendle concept to its ArcX equivalent.
</Info>
...
**What the equations share:** Both define a conservation law. The value of the parts equals the value of the whole. Both enable "upward arbitrage": if the tokens' combined market price exceeds the underlying, someone can mint and sell at a profit.

**Where they diverge:** Pendle's equation is reversible (mint and redeem at any time). ArcX's equation is one-directional (mint only, no early redemption). This single difference cascades through the entire pricing structure.
...
**In plain English:** Pendle splits *what an asset earns* (yield). ArcX splits *what a strategy earns* (PnL + points). Pendle wraps existing yield-bearing positions. ArcX wraps an actively managed trading strategy that runs on perp DEXes.
...
**Consequence:** The implied cost of EPT (via the flash loop) can diverge from theoretical fair value. This is structurally identical to how closed-end funds in TradFi trade at discounts to NAV.
...
**Honest assessment:** Pendle is significantly more trustless. ArcX trades trustlessness for access to off-chain perp DEX strategies and pre-TGE points markets. See [Trust Model & Security](/deep-dives/trust-model-and-security).
...
Instead of a tradeable YT with time-decay AMM pricing, ArcX uses a deposit-only EPT model where the flash loop replaces secondary market trading. This simplifies the system while still enabling leveraged points exposure.
...
Yes. They serve different purposes. Pendle is for yield optimization on DeFi assets. ArcX is for accessing and trading perp DEX strategy outcomes (PnL + points). A sophisticated farmer might use Pendle for fixed yield on staked assets and ArcX for points exposure on perp DEX strategies.
</Accordion>
</AccordionGroup>
