...
flowchart TB
    subgraph STARKNET["Starknet"]
        ...
        PT["PointsToken<br/>(per exchange, persists)"]
        RM["Redemption Module<br/>(post-TGE burn->claim)"]
        AMM["ArcX AMM<br/>(ST/USDC, Pendle-style)"]
        PC["PointsCollector<br/>(LP credit distribution)"]
    ...
    subgraph OFFCHAIN["Off-Chain (ArcX)"]
        direction TB
        NAV["NAV Oracle<br/>(every 5 min)"]
        CR["Credits Oracle<br/>(activity-weighted)"]
        FP["Final Points Oracle<br/>(at finalization)"]
        SR["Strategy Runner<br/>(perp DEX trading)"]
    end
