...
**The core idea:** EPT doesn't trade on a market. Its value comes from the flash loop: deposit USDC, sell ST on the ArcX AMM, keep EPT. The implied cost per EPT is `1 - X/R` where X = ST market price and R = exchange rate. This page derives the formula, explains what drives the ST discount, and shows how the system reaches equilibrium.
</Info>

**Prerequisites:** [The Three Tokens](/learn/token-economics), [Credit Mathematics](/deep-dives/credit-mathematics)
...
This is a **physical law** of the protocol. It's enforced by the deposit contract. When you deposit 1 USDC (ignoring fees for clarity), you receive exactly `1/R` ST shares and exactly 1 EPT. Always.
...
**In plain English:** You deposit \$1, receive ST + EPT, then sell the ST on the ArcX AMM. The EPT's cost is whatever USDC you don't recover from selling the ST. If your ST sale returns \$0.92, the EPT cost you \$0.08.
...
The flash loop is the primary mechanism for acquiring EPT. Understanding its economics is key to understanding EPT pricing.
...
**Key insight:** The flash loop is a capital multiplier, not a cost reducer. Each \$1 of capital buys EPT at the same effective cost. More iterations just let you deploy more of your initial capital into EPT.
...
ST's discount is structurally identical to a CEF discount. The key difference: ArcX's time-decay AMM curve provides a built-in convergence mechanism that CEFs lack. ST *will* converge to NAV at maturity, making the discount a known, bounded opportunity for yield seekers.
...
**The key structural difference:** Pendle YT decays to \$0 because all yield has been streamed by maturity. ArcX EPT doesn't trade at all --- its cost is determined indirectly through the ST discount on the ArcX AMM. This is simpler (no EPT AMM needed) but means EPT cost depends entirely on ST market dynamics.
...
It's a feature, not a bug. The discount is what makes the two-sided market work. Points farmers need someone to buy their ST. Yield seekers need a discount to earn returns. The discount is the price that clears the market between these two groups. A "correct" discount is one where both sides are satisfied.
</Accordion>
</AccordionGroup>
...
<Note>All rows show the same number of iterations (3). The key difference is **cost per EPT**: at a 5% discount, each EPT costs ~$0.05, while at a 15% discount, each costs ~$0.15. Deeper discounts mean you pay more per EPT but get more ST proceeds per loop.</Note>
...
**Key insight:** The discount provides a buffer against strategy losses. At a 10% discount, the strategy can lose up to 10% of NAV before your ST investment goes underwater. This is why yield seekers tolerate the strategy risk: the discount compensates them.
