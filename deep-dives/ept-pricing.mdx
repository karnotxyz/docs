...
This is not a theoretical relationship. Cross-matching enforces it mechanically. If the combined buy-side prices on both orderbooks exceed the minting cost, the matching engine triggers a mint and self-corrects the spread.
...
### Downward Arb Is Broken

If the combined prices fall below minting cost — meaning EPT and ST are both "cheap" — the correcting trade would require buying both tokens and burning them for USDC. **This is not possible.** There is no early redemption mechanism. You cannot burn ST + EPT before finalization.

The ST discount can therefore persist wider than fair value with no mechanical correction. Yield seekers buying cheap ST provide a natural floor, but there is no guaranteed arbitrage on the downside.

<Info>
No early redemption is a deliberate design choice. It eliminates complex reserve management and lets the full deposit work in the strategy for the entire epoch. Early redemption is on the roadmap for a future version.
</Info>
...
| **Strategy OI** | More open interest = more credits per tick = higher EPT price |
| **ST discount** | Higher ST discount = higher implied EPT cost (via minting parity) |
| **Points demand** | More buyers on EPT orderbook = higher price |

The relationship between ST discount and EPT cost deserves emphasis. From minting parity:

$$\text{EPT}_{\text{fair}} = 1 - \frac{X}{R}$$

A deeper ST discount (lower $X$) means a **higher** EPT fair value. Points farmers pay more for EPT when ST is cheap, because less of the \$1 minting cost is recovered from the ST side.
...
The same \$100 buys radically different exposure depending on which token you choose. ArcX does not create value — it separates it. The points farmer concentrates into points. The yield seeker concentrates into USDC returns. The direct depositor gets the unseparated bundle.
