...
|---|---|---|---|---|
| 1 | **NAV reporting** | ArcX misreports strategy value to favor certain depositors | Compare reported NAV against exchange position dashboards | Third-party attestation / audits |
...
| 4 | **Points backing** | ArcX over-mints PointsTokens beyond real points held | Limited. Exchange points are per-account, not publicly queryable | Exchange APIs or third-party attestation |
...
ArcX trades trustlessness for speed to market. Every trust assumption is documented here, and every oracle is operated by ArcX. Decentralization is planned, but today, you are trusting ArcX.

```mermaid
flowchart TB
    subgraph ON_CHAIN["On-Chain (Verifiable)"]
        direction TB
        ST["ST Token<br/>(balances, transfers)"]
        EPT["EPT Token<br/>(balances, credits)"]
        PT["PointsToken<br/>(supply, burns)"]
        ESC["Cross-Chain Escrow<br/>(locked balances)"]
    end

    subgraph OFF_CHAIN["Off-Chain (Trust Required)"]
        direction TB
        NAV["NAV Oracle<br/>(strategy value)"]
        CR["Credits Oracle<br/>(creditRate)"]
        FP["Final Points Oracle<br/>(totalPoints)"]
        STRAT["Strategy Execution<br/>(perp DEX trading)"]
    end
...
<Tabs>
  <Tab title="Oracle Integrity (NAV)">
...
Exchange APIs for points verification (if exchanges build them), or third-party attestation of account state. Direct exchange-to-contract distribution would eliminate the custody step entirely.
...
For emergency procedures (deposit pausing, what happens if ArcX or Starknet goes down), see [Epoch Lifecycle: Emergency](/learn/epoch-lifecycle#emergency-pause).
...
<Steps>
  ...
  <Step title="Cross-Reference Points">
  ...
  <Step title="Watch Escrow Balances">
  ...
  <Step title="Compare ST Redemption Values">
...
|---|---|---|---|
| NAV reporting | ArcX oracle | Community monitoring dashboards | Third-party attestation / audits |
