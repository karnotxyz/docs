...
<Info>
**The core idea:** ArcX settles tokens and balances on-chain, but runs strategy execution and data reporting off-chain. Every off-chain component has a specific practical reason for being off-chain, a defined protection mechanism, and a path toward decentralization.
...
The critical distinction: ArcX does not hold your tokens. ST, EPT, and PointsToken are standard tokens on Starknet. Your balances are yours. ArcX's oracles feed *data* into the contracts (NAV, credit rate, total points), but the contracts enforce the math. No oracle input can directly transfer your tokens to someone else.
...
ArcX runs five components off-chain. Each one is off-chain for a specific practical reason, not because of a preference for centralization. Click through each tab below to understand what the component does and why it cannot currently run on-chain.

<Tabs>
  <Tab title="Matching Engine">
  ...
  <Tab title="Strategy Execution">
    ...
    **The pattern:** ArcX executes trades on external exchanges using deposited capital. The results (PnL, points) are reported back to the on-chain contracts via oracles.
  </Tab>
  <Tab title="NAV Oracle">
    ...
    **Why it's off-chain:** NAV requires reading the strategy's account positions across multiple perp DEXes. These positions live in exchange accounts, not on-chain. No third-party oracle (Chainlink, Pyth) can compute strategy NAV because it requires access to ArcX's exchange account data. This is fundamentally different from price feeds, which aggregate public market data.
  </Tab>
  <Tab title="Credits Oracle">
    **What it does:** Reports the credit rate that determines how fast EPT holders accumulate credits. Credit rate reflects strategy activity --- open interest, volume, and other metrics that drive exchange point generation.

    **Why it's off-chain:** Credit rate derives from the strategy's trading activity on exchange accounts. OI and volume data come from the exchanges, not from on-chain sources. Until exchanges expose verifiable activity feeds, this data must be reported by ArcX.
  </Tab>
  <Tab title="Points Oracle">
    ...
    **Why it's off-chain:** Exchange points are tracked per-account on the exchange platform. They are not public on-chain data. ArcX reads its own account's points balance and reports it to the contract.
  </Tab>
</Tabs>
...
**Verification:** NAV is derived from positions on perp DEXes that have public dashboards. While you cannot see ArcX's exact account, the strategy's expected exposure (e.g., a funding arb with a stated capital deployment) is checkable against exchange data. NAV trajectory over an epoch should be consistent with observable market conditions --- a funding arb should track funding rates, a directional strategy should track the underlying asset.
...
**Protection:** The credit system has a mathematical fairness guarantee. Your share of total points equals your share of total credits, regardless of the credit rate schedule. Variable rates change *when* credits accrue fastest, not *whether* the allocation is fair. See [Credit Mathematics](/deep-dives/credit-mathematics) for the formal proof.

**Verification:** Credit rate derives from observable strategy activity metrics (OI, volume) on exchange dashboards. If ArcX reports a high credit rate but the strategy has minimal visible OI, the discrepancy is detectable. The inputs are not secret --- they correspond to publicly observable exchange activity.

**Bounding:** Even a multi-hour staleness produces modest distortion unless strategy activity changes dramatically during the outage. When the oracle resumes, the new rate applies going forward. Past accruals are already settled on-chain, so correcting forward does not retroactively alter prior balances.
...
**Bounding:** The refund timeout ensures that no funds can be locked indefinitely. The worst case is a temporary delay (the timeout period) before you can self-rescue. This is a safety net, not a normal flow --- the vast majority of cross-chain deposits complete within minutes.
...
| **Matching engine downtime** | Orders cannot execute on EPT/USDC or ST/USDC orderbooks | Missed trading opportunities | On-chain settlement ensures no fund loss during downtime. Existing positions are unaffected. |
...
| **Exchange risk** | A perp DEX suffers an exploit, freeze, or insolvency | Strategy capital on that exchange is at risk | ArcX diversifies across exchanges where possible. Epoch isolation limits exposure duration. |
...
What you can check yourself to independently audit ArcX's behavior. None of these checks require special access or ArcX's cooperation --- they use publicly available on-chain data and exchange information.
...
The key boundary: token mechanics (minting, burning, credit accrual, redemption) are fully on-chain. The *inputs* to those mechanics (NAV, credit rate, total points) are reported by ArcX oracles. Settlement enforces that oracle-reported values flow through on-chain logic, not arbitrary off-chain decisions.

Notice that arrows only flow in one direction: from off-chain to on-chain. Off-chain components *report data* to on-chain contracts. They do not directly control token balances, transfers, or settlement. The contracts use reported data to compute share prices, credit accrual, and point allocations --- but the computation itself is on-chain and deterministic.

---

## Decentralization Roadmap

ArcX is designed for progressive decentralization. Each off-chain component has a planned path toward reduced trust. The goal is not decentralization for its own sake, but reducing the number of things users must trust ArcX to do correctly.

| Component | Current State | Next Step | End State |
|---|---|---|---|
| **NAV** | ArcX oracle | Community monitoring dashboards | Third-party attestation or ZK proofs of exchange balances |
| **Credits** | ArcX oracle | On-chain staleness checks | Verifiable activity feeds from exchanges |
| **Points** | ArcX oracle | Public attestation of points balances | Exchange-native on-chain verification |
| **Strategy** | Trust ArcX execution | On-chain constraints (max leverage, drawdown triggers) | Isolated vaults with insurance |
...
- **Near-term** (no external dependencies): Community monitoring dashboards, on-chain staleness checks for the credits oracle, public attestation of points balances. These require only ArcX engineering effort.
...
How does ArcX's trust model compare to other ways of earning yield or farming points? The table below compares along the dimensions that matter most: where execution trust lies, how you exit, what data you can verify, and what your worst case looks like.

| | **ArcX** | **Pendle** | **Yield Aggregator** (e.g., Yearn) | **CEX Earn** |
|---|---|---|---|---|
| **Strategy execution** | ArcX (off-chain, perp DEXes) | N/A (no strategy) | Strategist / governance | Exchange |
...
| **Yield / points data** | ArcX oracles | On-chain (SY wrapper) | On-chain (vault NAV) | Exchange-internal |
...
ArcX trades full on-chain verifiability for access to off-chain yield sources --- perp DEX strategies and exchange points programs that do not exist on-chain. Fully on-chain protocols like Pendle work because their underlying yield is itself on-chain. ArcX's off-chain components exist because perp DEX trading is inherently off-chain.

The tradeoff is intentional: more trust, but access to yield sources that pure on-chain protocols cannot reach. As perp DEXes and exchange point programs evolve toward more transparent and on-chain-verifiable architectures, ArcX's trust surface will shrink accordingly. The decentralization roadmap above tracks this progression.

<Accordion title="How does this compare to Pendle's risk profile?">
Different risk profile, not strictly more or less risky. Pendle is fully on-chain --- you trust only smart contract risk. ArcX adds oracle risk (trusting ArcX's data feeds) and strategy risk (trusting ArcX's execution). In exchange, ArcX gives you access to perp DEX strategy returns and pre-TGE points markets that Pendle does not offer. The additional trust assumptions are the cost of accessing off-chain yield sources.
</Accordion>

<Accordion title="Why can't ArcX use Chainlink or Pyth for the NAV oracle?">
No exchange exposes a "strategy NAV" data feed. NAV is computed from ArcX's own exchange positions --- principal plus unrealized PnL across multiple perp DEXes. No third-party oracle can compute this without access to ArcX's exchange accounts. This is fundamentally different from price feeds, which aggregate public market data.
</Accordion>

<Accordion title="What happens if ArcX goes offline?">
...
<Accordion title="What if a perp DEX that ArcX trades on gets exploited?">
...
<Accordion title="What is my maximum possible loss?">
Your maximum loss is your deposit amount. ST can go to zero if the strategy loses 100% of capital. There is no margin call and no additional liability beyond what you deposited. Even in this worst case, your EPT credits and PointsToken allocation are unaffected --- points are earned during the epoch regardless of strategy PnL. You still claim your full PointsToken allocation even if ST redeems for zero.
</Accordion>

<Accordion title="Is ArcX audited?">
Contract addresses and audit status will be published in the [Developer Integration Guide](/build/developer-integration-guide). On-chain contracts on Starknet are verifiable through standard block explorer tools. Smart contract risk is present in any DeFi protocol, including ArcX.
</Accordion>
