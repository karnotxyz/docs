...
Each series is divided into equal-sized **credit periods**. These periods are chosen to match the points cycle on the underlying exchange.

So if an exchange distributes points every week, the protocol uses one-week credit periods on-chain.

> A credit period is the on-chain period whose accumulated credits receive CreditTokens for the corresponding points period.
