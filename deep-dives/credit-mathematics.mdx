...
description: "How the credit system ensures fair, time-and-activity-weighted points distribution."
icon: "calculator"
---
<Info>
**Course level: Advanced**

**The core idea:** Wondering why someone who held EPT the entire epoch got more points than someone who deposited on Day 40? Credits are the answer. They convert "how long you held" and "how active the strategy was" into a fair share of points. This page has the full math.
</Info>
...
**In plain English:** Your credits are the area under the curve of `(your EPT balance) x (the credit rate)` over time. More EPT, higher rate, longer duration: all increase your credits.
...
*The shaded area represents Alice's credits. She held 100 EPT at creditRate=1 for weeks 1--4, then transferred 50 EPT, holding 50 for weeks 5--8. Her total credits are proportional to this area.*
...
With constant rate, EPT's only value driver is time remaining. The market can't distinguish "strategy is crushing it" from "strategy is idle."
...
**Key insight:** `globalCreditIndex` increases monotonically. The difference between two snapshots of globalCreditIndex tells you how many credit-seconds per EPT accrued in that interval. Multiply by the user's balance, and that's how many credits they earned.
...
### Example 1: Single Holder, Constant Rate
...
### Example 2: Two Holders, No Transfers, Constant Rate
...
### Example 3: Mid-Epoch Transfer, Constant Rate
...
### Example 4: Mid-Epoch Transfer WITH Variable Rate
...
### Example 5: Smart Contract Holding EPT
...
**Key takeaway:** The contract "absorbs" credits for the period it held EPT. This is correct: those credits belong to whoever controls the contract, not to Alice after she transferred EPT. If the contract is an ArcX AMM pool holding ST/USDC liquidity that happens to receive EPT, the PointsCollector mechanism ensures these credits reach the actual participants.

### Example 6: The Last-Second Depositor Attack (Defeated)
...
ArcX's credit system is architecturally similar to the interest accrual patterns used across DeFi (Aave's liquidity index, Compound's exchange rate). The key difference: credits don't represent real yield. They represent a *claim on a future discrete allocation* (points), not a *continuous stream of real tokens*.
